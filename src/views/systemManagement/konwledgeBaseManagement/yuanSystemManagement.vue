...
const url = '/basic/yuanSystemAccess/selects'
export default {
  directives: { elDragDialog },
  name: 'systemManagementKonwledgeBaseManagementYuanSystemManagement',
  components: {
    SearchForm,
    BasicTable,
    UpdateForm
  ...
  methods: {
    ...
    getAddForm() {
      if (this.updateFormData === null) {
        ...
        baseRequest('/basic/yuanSystemAccess/getAddDate', param).then(response => {
          response.data.item = {}
          const result = response.data
          result.formConfig = JSON.parse(result.formConfig)
